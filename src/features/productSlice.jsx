...
export const getAllCategoryItem = createAsyncThunk('categoryItem' , async(categoryItems)=> {
  const res = await axios.get(`https://fakestoreapi.com/products${categoryItems}`);
  return res.data
})
export const sortAllItem = createAsyncThunk('sortItem' , async(sortItems)=> {
  const res = await axios.get(`https://fakestoreapi.com/products?sort=${sortItems}`);
  return res.data
})
